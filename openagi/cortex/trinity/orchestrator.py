...
async def run_full_trinity_pipeline(
    ...
    proposal_res = await generate_role_response(TrinityRole.EXPANDER, task, model)
    ...
    audit_res = await generate_role_response(TrinityRole.AUDITOR, task, model)
    ...
    decision_res = await generate_role_response(TrinityRole.GOVERNOR, task, model)
