...
AUDITOR_C_PROMPT = _auditor_prompt(
    letter="C",
    style="深思派（Opus 位）。追问底层假设、长期影响、用户没说出口的隐性目标。问题清单偏向『CEO 没考虑到的二阶效应』。"
)
...
- 任两路审计总分差 > {CONFLICT_THRESHOLD_HALT} 分 → 强制暂停，向用户汇报请求裁决（不要私自定稿）
"""
