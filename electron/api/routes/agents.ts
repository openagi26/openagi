    ...
    if (pid) {
      ...
      try {
        if (process.platform === 'darwin' || process.platform === 'linux') {
          // MUST use -sTCP:LISTEN. Otherwise lsof returns the client process (OpenAGI itself) 
...
export async function handleAgentRoutes(
  ...
  if (url.pathname === '/api/agents' && req.method === 'POST') {
    try {
      const body = await parseJsonBody<{ name: string; inheritWorkspace?: boolean; systemPrompt?: string }>(req);
      const snapshot = await createAgent(body.name, { inheritWorkspace: body.inheritWorkspace, systemPrompt: body.systemPrompt });
  ...
  if (url.pathname.startsWith('/api/agents/') && req.method === 'GET') {
    ...
    if (parts.length === 2 && parts[1] === 'prompt') {
      try {
        ...
        sendJson(res, 200, { success: true, prompt });
      } catch (error) {
        sendJson(res, 500, { success: false, error: String(error) });
      }
      return true;
  ...
  if (url.pathname.startsWith('/api/agents/') && req.method === 'PUT') {
    ...
    if (parts.length === 2 && parts[1] === 'prompt') {
      try {
        const body = await parseJsonBody<{ systemPrompt: string }>(req);
        const agentId = decodeURIComponent(parts[0]);
        if (!/^[\w-]+$/.test(agentId)) {
          sendJson(res, 400, { success: false, error: 'Invalid agent ID' });
          return true;
        }
        const snapshot = await updateAgentPrompt(agentId, body.systemPrompt ?? '');
