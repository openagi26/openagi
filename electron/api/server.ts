...
import { handleTrinityRoutes } from './routes/trinity';
...
export function startHostApiServer(ctx: HostApiContext, port = getPort('OPENAGI_HOST_API')): Server {
  ...
  server.on('error', (error: NodeJS.ErrnoException) => {
    if (error.code === 'EACCES' || error.code === 'EADDRINUSE') {
      logger.error(
        ...
        `Set OPENAGI_PORT_OPENAGI_HOST_API env var to override the default port.`,
      );
    } else {
