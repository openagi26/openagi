 ...
 * Manages the OpenAGI Gateway process lifecycle
 */
...
/**
 * Gateway Manager
 * Handles starting, stopping, and communicating with the OpenAGI Gateway
 */
export class GatewayManager extends EventEmitter {
  ...
  private async initDeviceIdentity(): Promise<void> {
    ...
    try {
      const identityPath = path.join(app.getPath('userData'), 'openagi-device-identity.json');
   ...
   * Uses OpenAGI protocol format: { type: "req", id: "...", method: "...", params: {...} }
   */
  async rpc<T>(method: string, params?: unknown, timeoutMs = 30000): Promise<T> {
    return new Promise((resolve, reject) => {
      ...
      // Send request using OpenAGI protocol format
      const request = {
        type: 'req',
        id,
        method,
   ...
   * OpenAGI Gateway doesn't have an HTTP /health endpoint
   */
  ...
  /**
   * Start Gateway process
   * Uses OpenAGI npm package from node_modules (dev) or resources (production)
   */
  ...
  private handleMessage(message: unknown): void {
    ...
    // Handle OpenAGI protocol response format: { type: "res", id: "...", ok: true/false, ... }
    ...
    // Handle OpenAGI protocol event format: { type: "event", event: "...", payload: {...} }
