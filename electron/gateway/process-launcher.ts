...
(function () {
  ...
  if (globalThis.__openagiFetchPatched) return;
  globalThis.__openagiFetchPatched = true;

  globalThis.fetch = function openagiFetch(input, init) {
    var url =
    ...
    if (url.indexOf('openrouter.ai') !== -1) {
      ...
      delete flat['x-title'];
      delete flat['X-Title'];
      flat['HTTP-Referer'] = 'https://openagi.app';
      flat['X-Title'] = 'OpenAGI';
      init.headers = flat;
    }
  ...
  if (process.platform === 'win32') {
    try {
      ...
      if (!cp.__openagiPatched) {
        cp.__openagiPatched = true;
