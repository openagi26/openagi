...
export async function runOpenClawDoctorRepair(): Promise<boolean> {
  ...
  if (!existsSync(entryScript)) {
    logger.error(`Cannot run OpenAGI doctor repair: entry script not found at ${entryScript}`);
    return false;
  }
  ...
  logger.info(
    `Running OpenAGI doctor repair (entry="${entryScript}", args="${doctorArgs.join(' ')}", cwd="${openclawDir}", bundledBin=${binPathExists ? 'yes' : 'no'})`,
  );
  ...
  return await new Promise<boolean>((resolve) => {
    ...
    const timeout = setTimeout(() => {
      logger.error('OpenAGI doctor repair timed out after 120000ms');
      try {
        child.kill();
      } catch {
        // ignore
    ...
    child.on('error', (err) => {
      clearTimeout(timeout);
      logger.error('Failed to spawn OpenAGI doctor repair process:', err);
      finish(false);
    });
    ...
    child.on('exit', (code: number) => {
      clearTimeout(timeout);
      if (code === 0) {
        logger.info('OpenAGI doctor repair completed successfully');
        finish(true);
        return;
      }
      logger.warn(`OpenAGI doctor repair exited (code=${code})`);
      finish(false);
    });
  });
}
