...
// Resolve dependencies from OpenAGI package context (pnpm-safe)
...
// contains OpenAGI's own devDependencies — packages that are NOT deps of openclaw
...
function resolveOpenClawPackageJson(packageName: string): string {
    ...
    // 2. Fallback to project-level deps (works in dev mode for OpenAGI devDependencies)
    try {
        return projectRequire.resolve(specifier);
    } catch (err) {
        const reason = err instanceof Error ? err.message : String(err);
        throw new Error(
            `Failed to resolve "${packageName}" from OpenAGI context. ` +
