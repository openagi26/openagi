/**
 * OpenAGI CLI utilities — cross-platform auto-install
 */
...
export function getOpenClawCliCommand(): string {
  ...
  if (platform === 'darwin' || platform === 'linux') {
    const localBinPath = join(homedir(), '.local', 'bin', 'openagi');
  ...
  if (platform === 'linux') {
    if (existsSync('/usr/local/bin/openagi')) {
      return '/usr/local/bin/openagi';
    }
  }
  ...
  if (!app.isPackaged) {
    ...
    const binName = platform === 'win32' ? 'openagi.cmd' : 'openagi';
  ...
  if (app.isPackaged) {
    if (platform === 'win32') {
      ...
      const cmdPath = join(cliDir, 'openagi.cmd');
...
function getPackagedCliWrapperPath(): string | null {
  ...
  if (platform === 'darwin' || platform === 'linux') {
    const wrapper = join(process.resourcesPath, 'cli', 'openagi');
    return existsSync(wrapper) ? wrapper : null;
  }
  if (platform === 'win32') {
    const wrapper = join(process.resourcesPath, 'cli', 'openagi.cmd');
    return existsSync(wrapper) ? wrapper : null;
  }
  return null;
}
...
function getCliTargetPath(): string {
  return join(homedir(), '.local', 'bin', 'openagi');
}
...
export async function installOpenClawCli(): Promise<{
  ...
  try {
    ...
    logger.info(`OpenAGI CLI symlink created: ${target} -> ${wrapperSrc}`);
    return { success: true, path: target };
  } catch (error) {
    logger.error('Failed to install OpenAGI CLI:', error);
...
function isCliInstalled(): boolean {
  ...
  if (platform === 'linux' && existsSync('/usr/local/bin/openagi')) return true;
