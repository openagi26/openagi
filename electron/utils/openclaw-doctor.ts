...
function appendDoctorOutput(
  ...
  logger.warn(
    `OpenAGI doctor ${stream} exceeded ${MAX_DOCTOR_OUTPUT_BYTES} bytes; truncating additional output`,
  );
...
async function runDoctorCommandWithArgs(
  ...
  const command = `openagi ${args.join(' ')}`;
  const startedAt = Date.now();

  if (!existsSync(entryScript)) {
    const error = `OpenAGI entry script not found at ${entryScript}`;
    logger.error(`Cannot run OpenAGI doctor: ${error}`);
    return {
      mode,
      success: false,
      exitCode: null,
  ...
  logger.info(
    `Running OpenAGI doctor (mode=${mode}, entry="${entryScript}", args="${args.join(' ')}", cwd="${openclawDir}", bundledBin=${binPathExists ? 'yes' : 'no'})`,
  );
  ...
  return await new Promise<OpenClawDoctorResult>((resolve) => {
    ...
    const timeout = setTimeout(() => {
      logger.error(`OpenAGI doctor timed out after ${OPENCLAW_DOCTOR_TIMEOUT_MS}ms`);
      try {
        child.kill();
      } catch {
        // ignore
    ...
    child.on('error', (error) => {
      ...
      logger.error('Failed to spawn OpenAGI doctor process:', error);
      finish({
        mode,
        success: false,
        exitCode: null,
    ...
    child.on('exit', (code) => {
      clearTimeout(timeout);
      logger.info(`OpenAGI doctor exited with code ${code ?? 'null'}`);
      finish({
        mode,
        success: code === 0,
        exitCode: code,
