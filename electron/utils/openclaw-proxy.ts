...
interface SyncProxyOptions {
   ...
   * currently disabled in OpenAGI settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync OpenAGI global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because OpenAGI proxy is disabled and preserve mode is enabled');
      return;
    }
