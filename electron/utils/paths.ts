...
function getElectronApp() {
  ...
  const fallbackUserData = process.env.OPENAGI_USER_DATA_DIR?.trim() || join(homedir(), '.openagi');
...
/**
 * Get OpenAGI config directory
 */
export function getOpenAGIConfigDir(): string {
  return join(homedir(), '.openagi');
}

/**
 * Get OpenAGI logs directory
 */
