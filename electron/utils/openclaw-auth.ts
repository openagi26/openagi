...
/**
 * Scan OpenAGI's bundled extensions directory to find all plugins that have
...
/**
 * Save an OAuth token to OpenAGI's auth-profiles.json.
 */
export async function saveOAuthTokenToOpenClaw(
  ...
  console.log(`Saved OAuth token for provider "${provider}" to OpenAGI auth-profiles (agents: ${agentIds.join(', ')})`);
}

/**
 * Retrieve an OAuth token from OpenAGI's auth-profiles.json.
...
/**
 * Save a provider API key to OpenAGI's auth-profiles.json
 */
export async function saveProviderKeyToOpenClaw(
  ...
  console.log(`Saved API key for provider "${provider}" to OpenAGI auth-profiles (agents: ${agentIds.join(', ')})`);
}

/**
 * Remove a provider API key from OpenAGI auth-profiles.json
 */
export async function removeProviderKeyFromOpenClaw(
  ...
  console.log(`Removed API key for provider "${provider}" from OpenAGI auth-profiles (agents: ${agentIds.join(', ')})`);
}
  ...
  try {
    await withConfigLock(async () => {
      ...
      if (entries[pluginName]) {
        ...
        modified = true;
        console.log(`Disabled OpenAGI plugin: ${pluginName}`);
      }
      ...
      if (providers[provider]) {
        delete providers[provider];
        modified = true;
        console.log(`Removed OpenAGI provider config: ${provider}`);
      }
      ...
      if (authProfiles) {
        for (const [profileId, profile] of Object.entries(authProfiles)) {
          ...
          modified = true;
          console.log(`Removed OpenAGI auth profile: ${profileId}`);
        }
      }
...
export async function setOpenClawDefaultModel(
  ...
  return withConfigLock(async () => {
    ...
    await writeOpenClawJson(config);
    console.log(`Set OpenAGI default model to "${model}" for provider "${provider}"`);
  });
}
...
export async function setOpenClawDefaultModelWithOverride(
  ...
  return withConfigLock(async () => {
    ...
    await writeOpenClawJson(config);
    console.log(
      `Set OpenAGI default model to "${model}" for provider "${provider}" (runtime override)`
    );
  });
}
 ...
 * Used by OpenAGI to seed the provider store when it's empty but providers are
...
/**
 * Write the OpenAGI gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  return withConfigLock(async () => {
    ...
    // Packaged OpenAGI loads the renderer from file://, so the gateway must allow
 ...
 * which means conversations disappear after roughly one day.  OpenAGI sets
 ...
 *
 * Removes known-invalid keys that cause OpenAGI's strict Zod validation
 ...
 * OpenClaw/OpenAGI versions or external tools.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    // ── skills section ──────────────────────────────────────────────
    // OpenAGI's Zod schema uses .strict() on the skills object, accepting
    ...
    // for OpenAGI to properly integrate with its updated tool system.
    ...
    // OpenAGI is a local desktop app where the user is the trusted operator.
    ...
    // OpenAGI's minSecurity/maxAsk merge will still respect their intent.
    ...
    if (execConfig.security !== 'full' || execConfig.ask !== 'off') {
      ...
      toolsModified = true;
      console.log('[sanitize] Set tools.exec.security="full" and tools.exec.ask="off" to disable exec approvals for OpenAGI desktop');
    }
    ...
    if (typeof plugins === 'object' && !Array.isArray(plugins)) {
      ...
      // OpenAGI's plugin enable logic checks the allowlist BEFORE enabledByDefault,
    ...
    // but OpenAGI historically stored them only under `channels.<type>.accounts.default`.
