...
async function provisionAgentFilesystem(
  ...
  options?: { inheritWorkspace?: boolean; systemPrompt?: string },
): Promise<void> {
  ...
  if (options?.systemPrompt?.trim()) {
    // systemPrompt优先级高于inheritWorkspace复制的SOUL.md：
    // 用户明确提供的角色设定应覆盖从主代理继承的SOUL.md。
    const soulPath = join(targetWorkspace, 'SOUL.md');
    await writeFile(soulPath, options.systemPrompt.trim(), 'utf-8');
  }
}
...
export async function updateAgentPrompt(agentId: string, systemPrompt: string): Promise<AgentsSnapshot> {
  return withConfigLock(async () => {
    const config = await readOpenClawConfig() as AgentConfigDocument;
    const { entries } = normalizeAgentsConfig(config);
    ...
    const targetWorkspace = expandPath(agent.workspace || `~/.openclaw/workspace-${agentId}`);
    await ensureDir(targetWorkspace);
    const soulPath = join(targetWorkspace, 'SOUL.md');
    // 无论 prompt 是否为空，均写入文件（空文件让 Gateway 以默认行为运行）
    // 不删除 SOUL.md，避免 Gateway 启动时行为不确定
    await writeFile(soulPath, systemPrompt.trim(), 'utf-8');
    logger.info('Updated agent prompt', { agentId });
    return buildSnapshotFromConfig(config);
  });
}
...
export async function getAgentPrompt(agentId: string): Promise<string> {
  ...
  const soulPath = join(targetWorkspace, 'SOUL.md');
  try {
    return (await readFile(soulPath, 'utf-8')).trim();
  } catch {
    return '';
  }
}
