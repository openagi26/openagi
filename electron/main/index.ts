 ...
 */
import { app, BrowserWindow, nativeImage, session, shell, systemPreferences } from 'electron';
...
import { createSpiritWindow, registerSpiritIpcHandlers } from '../windows/spirit';
import { warmupNetworkOptimization } from '../utils/uv-env';
import { initTelemetry } from '../utils/telemetry';

import { ClawHubService } from '../gateway/clawhub';
import { ensureOpenAGIContext, repairOpenAGIOnlyBootstrapFiles } from '../utils/openclaw-workspace';
...
const WINDOWS_APP_USER_MODEL_ID = 'app.openagi.desktop';
const isE2EMode = process.env.OPENAGI_E2E === '1';
const requestedUserDataDir = process.env.OPENAGI_USER_DATA_DIR?.trim();
...
// On Wayland this maps the running window to openagi.desktop (→ icon + app grouping);
// on X11 it supplements the StartupWMClass matching.
// Must be called before app.whenReady() / before any window is created.
if (process.platform === 'linux') {
  app.setDesktopName('openagi.desktop');
}
...
if (!gotElectronLock) {
  console.info('[OpenAGI] Another instance already holds the single-instance lock; exiting duplicate process');
  app.exit(0);
}
    ...
    if (!fileLock.acquired) {
      const ownerDescriptor = fileLock.ownerPid
        ...
        : fileLock.ownerFormat === 'unknown'
          ...
          : 'unknown owner';
      console.info(
        `[OpenAGI] Another instance already holds process lock (${fileLock.lockPath}, ${ownerDescriptor}); exiting duplicate process`,
      );
      app.exit(0);
    }
  } catch (error) {
    console.warn('[OpenAGI] Failed to acquire process instance file lock; continuing with Electron single-instance lock only', error);
  }
}
...
function createWindow(): BrowserWindow {
  ...
  const shouldSkipSetupForE2E = process.env.OPENAGI_E2E_SKIP_SETUP === '1';
  ...
  const win = new BrowserWindow({
    ...
    webPreferences: {
      ...
      sandbox: false,
      webviewTag: true, // Enable <webview> for embedding OpenAGI Control UI
    },
...
async function initialize(): Promise<void> {
  ...
  logger.init();
  logger.info('=== OpenAGI Application Starting ===');
  logger.debug(
  ...
  if (!isE2EMode) {
    try {
      createSpiritWindow(window);
      registerSpiritIpcHandlers(window);
      logger.info('小星浮窗创建成功');
    } catch (error) {
      logger.warn('小星浮窗创建失败:', error);
    }
  }

  // Override security headers ONLY for the OpenAGI Gateway Control UI.
  ...
  // Repair any bootstrap files that only contain OpenAGI markers (no OpenAGI
  // template content). This fixes a race condition where ensureOpenAGIContext()
  // previously created the file before the gateway could seed the full template.
  if (!isE2EMode) {
    void repairOpenAGIOnlyBootstrapFiles().catch((error) => {
  ...
  // Pre-deploy/upgrade bundled OpenAGI plugins (dingtalk, wecom, feishu, wechat)
  // to ~/.openclaw/extensions/ so they are always up-to-date after an app update.
  // Note: qqbot was moved to a built-in channel in OpenAGI 3.31.
  if (!isE2EMode) {
  ...
  gatewayManager.on('status', (status: { state: string }) => {
    ...
    if (status.state === 'running' && !isE2EMode) {
      void ensureOpenAGIContext().catch((error) => {
        logger.warn('Failed to re-merge OpenAGI context after gateway reconnect:', error);
      });
    }
  });
  ...
  // Merge OpenAGI context snippets into the workspace bootstrap files.
  // The gateway seeds workspace files asynchronously after its HTTP server
  // is ready, so ensureOpenAGIContext will retry until the target files appear.
  if (!isE2EMode) {
    void ensureOpenAGIContext().catch((error) => {
      logger.warn('Failed to merge OpenAGI context into workspace:', error);
    });
  }

  // Auto-install openagi CLI and shell completions (non-blocking).
  if (!isE2EMode) {
    void autoInstallCliIfNeeded((installedPath) => {
      mainWindow?.webContents.send('openagi:cli-installed', installedPath);
    }).then(() => {
...
if (gotTheLock) {
  ...
  app.on('second-instance', () => {
    logger.info('Second OpenAGI instance detected; redirecting to the existing window');
  ...
  app.whenReady().then(async () => {
    // W15：macOS 麦克风（microphone）权限申请
    if (process.platform === 'darwin') {
      const micStatus = systemPreferences.getMediaAccessStatus('microphone');
      if (micStatus !== 'granted') {
        try {
          await systemPreferences.askForMediaAccess('microphone');
          logger.info('[W15] 麦克风权限申请完成');
        } catch (e) {
          logger.warn('[W15] 麦克风权限申请失败（用户可稍后在系统设置中手动授权）:', e);
        }
      }

      // W16：macOS 摄像头（camera）权限申请
      const camStatus = systemPreferences.getMediaAccessStatus('camera');
      if (camStatus !== 'granted') {
        try {
          await systemPreferences.askForMediaAccess('camera');
          logger.info('[W16] 摄像头权限申请完成');
        } catch (e) {
          logger.warn('[W16] 摄像头权限申请失败（用户可稍后在系统设置中手动授权）:', e);
        }
      }
    }
