...
function registerCronHandlers(gatewayManager: GatewayManager): void {
  ...
  // UI-created tasks have no delivery target — results go to the OpenAGI chat page.
  ...
  ipcMain.handle('cron:create', async (_, input: {
    ...
    try {
      const gatewayInput = {
        ...
        wakeMode: 'next-heartbeat',
        sessionTarget: 'isolated',
        // UI-created jobs deliver results via OpenAGI WebSocket chat events,
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        console.log(`[openagi-validate] validating provider type: ${providerType}`);
...
function registerSessionHandlers(gatewayManager: GatewayManager): void {
  ...
  // ────────────────────────────────────────────────────────────────────
  // W16 视频拍照：spirit:photo-input
  // 收到渲染层（小星 VideoButton）发来的 base64 截图
  // → 附上图像内容，转发给支持视觉（vision）的 LLM（大语言模型）
  // → 如果模型不支持视觉，返回降级文案
  // ────────────────────────────────────────────────────────────────────
  ipcMain.handle('spirit:photo-input', async (_event, imageBase64: string) => {
    logger.info('[spirit:photo-input] 收到拍照截图，准备发给 LLM（大语言模型）');

    try {
      // 检查网关是否已连接
      const isConnected = gatewayManager?.isConnected?.() ?? false;
      if (!isConnected) {
        return '我拍到了你，但现在还连不上大脑，等网关修好才能告诉你我看到什么～';
      }

      // 提取 base64 数据部分（去掉 "data:image/xxx;base64," 前缀）
      const base64Data = imageBase64.replace(/^data:image\/\w+;base64,/, '');

      // 尝试通过 RPC（远程调用）发送视觉（vision）请求
      let rpcResult: unknown;
      try {
        rpcResult = await gatewayManager.rpc('chat.send', {
          message: '请用中文、自然亲切的语气描述你看到的画面，就像朋友聊天一样，两三句话即可。',
          session_id: 'spirit-vision',
          images: [base64Data],
        });
      } catch (visionErr) {
        // 视觉（vision）调用失败，说明当前模型不支持看图
        logger.warn('[spirit:photo-input] 视觉调用失败，降级提示：', visionErr);
        return '你拍了一张照片，但当前 AI 模型不支持看图。去设置里换个能看图的模型吧～';
      }
  ...
  // ────────────────────────────────────────────────────────────────────
  // W15 语音聊天：spirit:voice-input
  // 收到渲染层（小星 VoiceButton）发来的语音识别文字
  // → 转发给 openclaw chat（聊天），返回回复文字
  // → 如果网关（gateway）未通，返回降级文案
  // ────────────────────────────────────────────────────────────────────
  ipcMain.handle('spirit:voice-input', async (_event, text: string) => {
    logger.info(`[spirit:voice-input] 收到语音输入：${text}`);
