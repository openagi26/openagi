; NewClaw Custom NSIS Installer/Uninstaller Script
;
...
!macro customCheckAppRunning
  ...
  DetailPrint "Extracting NewClaw runtime files. This can take a few minutes on slower disks or while antivirus scanning is active."
  ...
  ${if} $R0 == 0
    ...
    # This covers NewClaw.exe (multiple Electron processes), openclaw-gateway.exe,
  ...
  ; Even if NewClaw.exe was not detected as running, orphan child processes
  ...
  ; PowerShell path-based kill may miss processes if the old NewClaw was installed
  ...
  ; "Can't modify NewClaw's files" retry loop -> "NewClaw 无法关闭" dialog.
  ;
  ...
  ; files (antivirus, indexing) causing a blocking "NewClaw 无法关闭" dialog.
...
; "NewClaw 无法关闭" retry dialog when the old uninstaller fails.
;
...
!macro customInstall
  ...
  ; Wait 60s before starting deletion to avoid I/O contention with NewClaw's
...
!macro customUnInstall
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "Do you want to remove NewClaw application data?$\r$\n$\r$\nThis will delete:$\r$\n  • AppData\Local\newclaw (local app data)$\r$\n  • AppData\Roaming\newclaw (roaming app data)$\r$\n$\r$\nYour .openclaw folder (configuration & skills) will be preserved.$\r$\nSelect 'No' to keep all data for future reinstallation." \
    /SD IDNO IDYES _cu_removeData IDNO _cu_skipRemove

  _cu_removeData:
    ; Kill any lingering NewClaw processes (and their child process trees) to
