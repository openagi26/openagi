...
# Post-installation script for NewClaw on Linux
...
# Create symbolic link for NewClaw app binary
if [ -x /opt/NewClaw/newclaw ]; then
    ln -sf /opt/NewClaw/newclaw /usr/local/bin/newclaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/NewClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/NewClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/NewClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/NewClaw/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/newclaw'
...
echo "NewClaw has been installed successfully."
