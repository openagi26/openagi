...
async function launchOpenAGIElectron(
  homeDir: string,
  userDataDir: string,
  ...
  return await electron.launch({
    ...
    env: {
      ...
      OPENAGI_E2E: '1',
      OPENAGI_USER_DATA_DIR: userDataDir,
      ...(options.skipSetup ? { OPENAGI_E2E_SKIP_SETUP: '1' } : {}),
      OPENAGI_PORT_OPENAGI_HOST_API: String(hostApiPort),
    },
    timeout: 90_000,
  });
}
...
export const test = base.extend<ElectronFixtures>({
  homeDir: async ({ browserName: _browserName }, provideHomeDir) => {
    const homeDir = await mkdtemp(join(tmpdir(), 'openagi-e2e-home-'));
  ...
  userDataDir: async ({ browserName: _browserName }, provideUserDataDir) => {
    const userDataDir = await mkdtemp(join(tmpdir(), 'openagi-e2e-user-data-'));
    try {
  ...
  launchElectronApp: async ({ homeDir, userDataDir }, provideLauncher) => {
    await provideLauncher(async (options?: LaunchElectronOptions) => await launchOpenAGIElectron(homeDir, userDataDir, options));
  },
