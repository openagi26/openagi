...
vi.mock('@electron/utils/paths', () => ({
  ...
  getOpenClawEntryPath: () => 'C:\\Program Files\\OpenAGI\\resources\\openclaw\\openclaw.mjs',
}));
...
describe('getOpenClawCliCommand (Windows packaged)', () => {
  beforeEach(() => {
    ...
    Object.defineProperty(process, 'resourcesPath', {
      value: 'C:\\Program Files\\OpenAGI\\resources',
      configurable: true,
      writable: true,
    });
  });
  ...
  it('prefers bundled node.exe when present', async () => {
    mockExistsSync.mockImplementation((p: string) => /[\\/]cli[\\/]openagi\.cmd$/i.test(p) || /[\\/]bin[\\/]node\.exe$/i.test(p));
    const { getOpenClawCliCommand } = await import('@electron/utils/openclaw-cli');
    expect(getOpenClawCliCommand()).toBe(
      "& 'C:\\Program Files\\OpenAGI\\resources/cli/openagi.cmd'",
    );
  });
  ...
  it('falls back to bundled node.exe when openclaw.cmd is missing', async () => {
    ...
    const { getOpenClawCliCommand } = await import('@electron/utils/openclaw-cli');
    expect(getOpenClawCliCommand()).toBe(
      "& 'C:\\Program Files\\OpenAGI\\resources/bin/node.exe' 'C:\\Program Files\\OpenAGI\\resources\\openclaw\\openclaw.mjs'",
    );
  });
  ...
  it('falls back to ELECTRON_RUN_AS_NODE command when wrappers are missing', async () => {
    ...
    expect(command.endsWith("'C:\\Program Files\\OpenAGI\\resources\\openclaw\\openclaw.mjs'")).toBe(true);
  });
});
