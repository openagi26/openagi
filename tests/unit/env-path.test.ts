...
describe('env-path', () => {
  ...
  it('prepends entry with Windows delimiter', () => {
    setPlatform('win32');
    const next = prependPathEntry({ Path: 'C:\\Windows\\System32' }, 'D:\\openagi\\resources\\bin');
    expect(next.path).toBe('D:\\openagi\\resources\\bin;C:\\Windows\\System32');
    expect(next.env.Path).toBe('D:\\openagi\\resources\\bin;C:\\Windows\\System32');
  });

  it('prepends entry with POSIX delimiter', () => {
    setPlatform('linux');
    const next = prependPathEntry({ PATH: '/usr/bin:/bin' }, '/opt/openagi/bin');
    expect(next.path).toBe('/opt/openagi/bin:/usr/bin:/bin');
    expect(next.env.PATH).toBe('/opt/openagi/bin:/usr/bin:/bin');
  });
});
