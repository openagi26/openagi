...
const { testHome, testUserData } = vi.hoisted(() => {
  ...
  return {
    testHome: `/tmp/openagi-openclaw-auth-${suffix}`,
    testUserData: `/tmp/openagi-openclaw-auth-user-data-${suffix}`,
  };
});
...
describe('saveProviderKeyToOpenClaw', () => {
  ...
  it('only syncs auth profiles for configured agents', async () => {
    ...
    expect(logSpy).toHaveBeenCalledWith(
      'Saved API key for provider "openrouter" to OpenAGI auth-profiles (agents: main, test3)',
    );
