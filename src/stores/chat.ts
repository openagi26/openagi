 ...
 * Communicates with OpenAGI Gateway via renderer WebSocket RPC.
 */
...
const IMAGE_CACHE_KEY = 'openagi:image-cache';
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  // ── Delete session ──
  //
  // NOTE: The OpenAGI Gateway does NOT expose a sessions.delete (or equivalent)
  ...
  loadHistory: async (quiet = false) => {
    ...
    const loadPromise = (async () => {
      ...
      // displayName (e.g. the configured agent name "OpenAGI") instead.
  ...
  sendMessage: async (
    ...
    try {
      ...
      if (hasMedia) {
        // 【Bug1 修复】不再把 system prompt 嵌入 user message，直接发原始消息
        // system prompt 通过 Settings 页面写入 agent 层（updateAgentPrompt），不走消息前缀
        const baseMessage = trimmed || 'Process the attached file(s).';
  ...
  renameSession: (sessionKey, label) =>
    set((s) => ({
      sessionLabels: {
        ...s.sessionLabels,
        [sessionKey]: label.trim() || s.sessionLabels[sessionKey] || sessionKey,
      },
    })),
}));
