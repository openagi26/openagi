...
export function createRuntimeSendActions(set: ChatSet, get: ChatGet): Pick<RuntimeActions, 'sendMessage' | 'abortRun'> {
  return {
    sendMessage: async (
      ...
      // 特殊处理：@all 广播到所有团队成员（并行直发，不触发session切换和UI重置）
      if (targetAgentId === '__all__') {
        const trimmedBroadcast = text.trim();
        if (!trimmedBroadcast && (!attachments || attachments.length === 0)) return;

        const allAgents = useAgentsStore.getState().agents;
        if (allAgents.length === 0) return;

        // 在当前session追加可见的广播用户消息，不切换session
        const broadcastNowMs = Date.now();
        const broadcastUserMsg: RawMessage = {
          role: 'user',
          content: trimmedBroadcast || '(file attached)',
          timestamp: broadcastNowMs / 1000,
          id: crypto.randomUUID(),
        };
        set((s) => ({
          messages: [...s.messages, broadcastUserMsg],
          sending: true,
          error: null,
        }));

        // 预缓存附件（附件路径需在所有agent的RPC中复用）
        const hasMediaBroadcast = attachments && attachments.length > 0;
        if (hasMediaBroadcast && attachments) {
          for (const a of attachments) {
            upsertImageCacheEntry(a.stagedPath, {
              fileName: a.fileName, mimeType: a.mimeType,
              fileSize: a.fileSize, preview: a.preview,
            });
          }
        }

        // 解析所有agent的目标session key，确保session条目存在
        const broadcastTargets = allAgents.map((agent) => ({
          agentId: agent.id,
          sessionKey: resolveMainSessionKeyForAgent(agent.id) ?? buildFallbackMainSessionKey(agent.id),
        }));
        set((s) => {
          let sessions = s.sessions;
          for (const t of broadcastTargets) {
            sessions = ensureSessionEntry(sessions, t.sessionKey);
          }
          return { sessions };
        });

        // 并行发送给所有agent，每个使用独立的idempotencyKey，互不阻塞
        const BROADCAST_TIMEOUT_MS = 120_000;
        const broadcastResults = await Promise.allSettled(
          broadcastTargets.map(async (t) => {
            try {
              if (hasMediaBroadcast && attachments) {
                return await invokeIpc('chat:sendWithMedia', {
                  sessionKey: t.sessionKey,
                  message: trimmedBroadcast || 'Process the attached file(s).',
                  deliver: false,
                  idempotencyKey: crypto.randomUUID(),
                  media: attachments.map((a) => ({
                    filePath: a.stagedPath, mimeType: a.mimeType, fileName: a.fileName,
                  })),
                }) as { success: boolean };
              } else {
                return await invokeIpc(
                  'gateway:rpc', 'chat.send',
                  { sessionKey: t.sessionKey, message: trimmedBroadcast, deliver: false, idempotencyKey: crypto.randomUUID() },
                  BROADCAST_TIMEOUT_MS,
                ) as { success: boolean };
              }
            } catch (err) {
              console.error(`[broadcast] Failed to send to ${t.agentId}:`, err);
              return { success: false };
            }
          })
        );

        const failCount = broadcastResults.filter(
          (r) => r.status === 'rejected' || (r.status === 'fulfilled' && !(r.value as { success: boolean })?.success)
        ).length;

        set({
          sending: false,
          error: failCount > 0 ? `广播完成，${failCount}/${allAgents.length} 个成员发送失败` : null,
        });
        return;
      }
