...
interface AgentsState {
  ...
  createAgent: (name: string, options?: { inheritWorkspace?: boolean; systemPrompt?: string }) => Promise<void>;
  updateAgent: (agentId: string, name: string) => Promise<void>;
  updateAgentModel: (agentId: string, modelRef: string | null) => Promise<void>;
  updateAgentPrompt: (agentId: string, systemPrompt: string) => Promise<void>;
...
export const useAgentsStore = create<AgentsState>((set) => ({
  ...
  createAgent: async (name: string, options?: { inheritWorkspace?: boolean; systemPrompt?: string }) => {
    set({ error: null });
    try {
      const snapshot = await hostApiFetch<AgentsSnapshot & { success?: boolean }>('/api/agents', {
        method: 'POST',
        body: JSON.stringify({ name, inheritWorkspace: options?.inheritWorkspace, systemPrompt: options?.systemPrompt }),
      });
  ...
  updateAgentPrompt: async (agentId: string, systemPrompt: string) => {
    set({ error: null });
    try {
      const snapshot = await hostApiFetch<AgentsSnapshot & { success?: boolean }>(
  ...
  getAgentPrompt: async (agentId: string): Promise<string> => {
    try {
      const result = await hostApiFetch<{ success: boolean; prompt: string }>(
        `/api/agents/${encodeURIComponent(agentId)}/prompt`
      );
