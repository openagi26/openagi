...
export function UpdateSettings() {
  ...
  const renderStatusText = () => {
    ...
    switch (status) {
      ...
      case 'error':
        // 隐藏 dev mode 无关紧要的错误，显示友好文案
        if (error && (error.includes('dev mode') || error.includes('without a result'))) {
          return '当前为开发版本，自动更新不可用';
        }
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Error Details — 隐藏 dev mode 无关紧要的错误 */}
      {status === 'error' && error && !error.includes('dev mode') && !error.includes('without a result') && (
