...
import { SendHorizontal, Square, X, Paperclip, FileText, Film, Music, FileArchive, File, Loader2, AtSign, Users } from 'lucide-react';
...
export function ChatInput({ onSend, onStop, disabled = false, sending = false, isEmpty = false }: ChatInputProps) {
  ...
  // @提及：过滤后的候选列表（仅在team模式下）
  const mentionCandidates = useMemo(() => {
    if (chatMode !== 'team') return [];
    const query = mentionQuery.toLowerCase();
    const allEntry = { id: '__all__', name: 'all', display: t('mention.all') };
    const agentEntries = mentionableAgents
      .filter((a) => a.name.toLowerCase().includes(query))
      .map((a) => ({ id: a.id, name: a.name, display: a.name }));
    if ('all'.includes(query)) {
      return [allEntry, ...agentEntries];
    }
    return agentEntries;
  }, [chatMode, mentionQuery, mentionableAgents, t]);
  ...
  // @提及选中回调
  const selectMention = useCallback((candidate: { id: string; name: string }) => {
    if (!textareaRef.current) return;
    const textarea = textareaRef.current;
    const cursorPos = textarea.selectionStart ?? input.length;
    // 找到@触发位置，替换 @query 为 @name（加一个空格）
    const before = input.substring(0, mentionTriggerPos);
    const after = input.substring(cursorPos);
    const newInput = `${before}@${candidate.name} ${after}`;
    setInput(newInput);
    setShowMentions(false);
    setMentionQuery('');
    setMentionTriggerPos(-1);
    // 将光标移动到插入文本末尾
    const newCursorPos = before.length + candidate.name.length + 2; // '@' + name + ' '
    setTimeout(() => {
      textarea.focus();
      textarea.setSelectionRange(newCursorPos, newCursorPos);
    }, 0);
  }, [input, mentionTriggerPos]);
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent) => {
      ...
      if (showMentions && mentionCandidates.length > 0) {
        if (e.key === 'ArrowDown') {
          e.preventDefault();
          const nextIndex = (mentionIndex + 1) % mentionCandidates.length;
          setMentionIndex(nextIndex);
          // 自动滚动高亮项到视野内
          setTimeout(() => {
            mentionsRef.current?.children[nextIndex]?.scrollIntoView({ block: 'nearest' });
          }, 0);
          return;
        }
        if (e.key === 'ArrowUp') {
          e.preventDefault();
          const prevIndex = (mentionIndex - 1 + mentionCandidates.length) % mentionCandidates.length;
          setMentionIndex(prevIndex);
          // 自动滚动高亮项到视野内
          setTimeout(() => {
            mentionsRef.current?.children[prevIndex]?.scrollIntoView({ block: 'nearest' });
          }, 0);
          return;
        }
    ...
    [handleSend, input, targetAgentId, showMentions, mentionCandidates, mentionIndex, selectMention],
  );
        ...
        <div className={`relative glass rounded-[28px] shadow-sm border p-1.5 transition-all ${dragOver ? 'border-primary ring-1 ring-primary' : 'border-black/10 dark:border-white/10'}`}>
          ...
          <div className="flex items-end gap-1.5">
            ...
            <div className="flex-1 relative">
              {/* @提及浮层（team模式） */}
              {showMentions && chatMode === 'team' && mentionCandidates.length > 0 && (
                <div
                  ref={mentionsRef}
                  className="absolute bottom-full left-0 w-64 mb-1 bg-popover border border-black/10 dark:border-white/10 rounded-xl shadow-lg max-h-48 overflow-y-auto z-50"
                  role="listbox"
                ...
                >
                  {mentionCandidates.map((candidate, i) => (
                    <button
                      ...
                      className={cn(
                        "w-full px-3 py-2 text-left text-sm flex items-center gap-2 hover:bg-black/5 dark:hover:bg-white/5 transition-colors",
                        i === mentionIndex && "bg-primary/10 text-foreground"
                      )}
                      onMouseDown={(e) => {
                        // 使用mousedown阻止textarea失焦
                        e.preventDefault();
                    ...
                    >
                      ...
                      {candidate.id === '__all__' && (
                        <span className="text-xs text-muted-foreground ml-auto">{candidate.display}</span>
                      )}
                    </button>
                  ))}
                </div>
              ...
              <Textarea
                ...
                onChange={(e) => {
                  ...
                  if (chatMode === 'team') {
                    ...
                    if (atMatch) {
                      ...
                      setMentionTriggerPos(cursorPos - atMatch[0].length);
                      setMentionIndex(0);
                ...
                onPaste={handlePaste}
                placeholder={disabled ? t('composer.gatewayDisconnectedPlaceholder') : (chatMode === 'team' ? t('mention.placeholder') : '')}
                disabled={disabled}
