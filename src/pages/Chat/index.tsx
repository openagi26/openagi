/**
 * Chat Page
 * Native React implementation communicating with OpenAGI Gateway
...
import { RunningAgentPlaceholder } from './RunningAgentPlaceholder';
import { AiMembersSidebar } from './AiMembersSidebar';
...
export function Chat() {
  ...
  useEffect(() => {
    if (sending && streamingTimestamp === 0) {
      ...
      setStreamingTimestamp(now / 1000);
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setRunStartTime(now);
  ...
  void (hasStreamText || hasStreamThinking || hasStreamTools || hasStreamImages || hasStreamToolStatus); // hasAnyStreamContent — kept for future fallback use

  // 解析当前 Agent 显示名称，agents 未加载时 fallback 为 "Agent"（不显示原始 ID）
  const currentAgent = agents.find((a) => a.id === currentAgentId);
  const currentAgentName = currentAgent?.name ?? 'Agent';
  // 团队模式下为 assistant 消息传入 Agent 归属信息
  const agentInfo = chatMode === 'team' && currentAgent
  ...
  return (
    <div className="flex h-full">
      ...
      <div
        className={cn(
          'relative flex flex-col -m-6 transition-colors duration-500 bg-transparent flex-1',
          // team模式下面板为flex子元素，无需margin
        )}
          ...
          <div ref={contentRef} className="max-w-4xl mx-auto space-y-4">
            {isEmpty ? (
              ...
              <>
                ...
                {/* RunningAgentPlaceholder: 等待 AI 响应或工具循环间歇期
                    error 由下方 Error bar 统一处理，不传入组件避免双重显示
                    thinkingContent 遵守 showThinking 开关 */}
                {sending && !shouldRenderStreaming && (
                  <RunningAgentPlaceholder
                    agentName={currentAgentName}
                    startTime={runStartTime || Date.now()}
                    thinkingContent={showThinking ? (streamThinking ?? undefined) : undefined}
                    streamingTools={streamingTools}
                    isStopping={false}
      ...
      {/* 团队协作面板（右侧） */}
      {chatMode === 'team' && (
        <div className="w-[280px] shrink-0 border-l bg-background overflow-hidden">
          <AiMembersSidebar className="h-full" />
        </div>
      )}
    </div>
  );
...
// ── Typing Indicator ── (保留作为 fallback，RunningAgentPlaceholder 未覆盖的场景备用)
...
// ── Activity Indicator (shown between tool cycles) ── (保留作为 fallback)
