      ...
      {!isUser && (
        <div className="flex flex-col items-center gap-1 shrink-0 mt-1">
          <div className={cn(
            ...
            agentInfo ? `${getAgentColor(agentInfo.id)} text-white` : 'bg-black/5 dark:bg-white/5',
          )}>
            {agentInfo ? (
              <span className="text-[11px] font-bold leading-none">{agentInfo.name.charAt(0).toUpperCase()}</span>
            ) : (
              <Sparkles className="h-4 w-4" />
            )}
          </div>
          {agentInfo && (
            <span className="text-[9px] text-muted-foreground font-medium truncate max-w-[3.5rem] leading-none">
              @{agentInfo.name}
            </span>
          )}
        </div>
